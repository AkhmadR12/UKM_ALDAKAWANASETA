                            ...
                            <div class="card">
                                <div class="card-header">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <a href="{{ route('testimoni.create') }}" class="btn btn-primary btn-round ms-auto">
                                        ...
                                        <table id="add-row" class="display table table-hover">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                      <button class="btn btn-sm toggle-status-btn {{ $testimoni->status === 'aktif' ? 'btn-success' : 'btn-danger' }}" 
                                                            data-id="{{ $testimoni->id }}" 
                                                            data-status="{{ $testimoni->status }}">
                                                        {{ ucfirst($testimoni->status) }}
                                                        </button>
                                                    </td>
                                                    <td>
                                                        <img src="{{ asset($testimoni->gambar) }}" alt="Member Image" width="100">
                                                    </td>                                                     
                                                    <td>{{ $testimoni->pekerjaan }}</td>
                                                    <td>{{ $testimoni->bintang }}</td>
    ...
    <script>
        $(document).ready(function() {
            $('.toggle-status-btn').on('click', function() {
                ...
                $.ajax({
                    url: '/testimoni/toggle-status/' + id,
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
